...
- 下一优先级是补充 RAG 多阶段 Trace：把当前检索结果记录细化为 parse、chunk、embed、retrieve、rerank、citation 阶段，并与 Retrieval Eval 关联；真实 `ModelClient` 适配器作为其后的独立阶段；
...
结合当前简历目标，Runtime 已具备节点级 Trace、输入输出映射、变量引用校验、最小连续多轮对话触发、Runtime Eval MVP 与 Web API 演示入口；RAG 已具备 Markdown 结构感知 Chunker 和确定性 Web 演示闭环。下一步不要急着扩展 `tool_call`、switch、loop 或并发，应优先补充 RAG 多阶段 Trace，再独立接入真实 `ModelClient` 适配器。
...
`{"user_input": "..."}`，成功时固定返回 `session_id`、`output_text`、顶层
`citations`、本轮 `node_traces` 和本轮 `tool_traces`。前端必须以顶层 `citations`
渲染参考来源，`tool_traces` 仅用于调试。Runtime 执行异常会写入服务端日志，并返回不含
内部细节的 `500` 错误：
...
共享、持久化恢复、鉴权、流式输出、真实模型或用户上传的 RAG 知识库。

### 10.1 已完成：确定性 Agentic RAG Web 演示闭环

默认 Web Demo 已把固定的 `demo_docs/` 演示知识库接入 Agent Loop：

```text
Demo Markdown
→ RawDocument / MarkdownDocumentParser
→ TextChunker / SimpleEmbeddingModel / InMemoryChunkIndex
→ RetrievalTool(retrieval.search)

用户消息
→ DemoRagPlanner
→ ToolAction(retrieval.search)
→ ToolExecutor / ToolTrace
→ observation
→ call_id 定位 Tool Trace
→ 确定性最终回答
→ 顶层 citations
```

文档入库链路仅在 `_get_demo_retrieval_tool()` 第一次调用时执行。该函数使用
`@lru_cache(maxsize=1)`，按文件名稳定排序加载 `demo_docs/*.md`，并使用基于 `__file__`
计算的绝对路径，因此每次聊天只查询已构建的索引，不会重复读取、解析、切分或向量化文档。
缓存中的 RetrievalTool 与索引在构建完成后只读，可以跨 session 共享；每次消息仍会为当前
SessionState 新建 Planner、TraceRecorder、ToolExecutor、Agent Loop 和 ConversationRuntime。

`DemoRagPlanner` 不保存回合游标。最后一条消息为用户消息时，它调用
`retrieval.search(query=user_input, top_k=3)`；最后一条消息为该工具 observation 时，它从
metadata 获取 `call_id`，再倒序精确查找同 call_id 的 Tool Trace，绝不解析 observation
文本或简单读取最新 Trace。成功检索使用前一至两个 Chunk 内容形成确定性回答；无命中与失败
分别返回稳定的用户提示。

Web 层只从 `ConversationTurnResult.tool_traces`（当前回合快照）提取成功
`retrieval.search` 的 `result.citations`。投影会过滤非法值、按 `(doc_id, chunk_id)` 去重、
保持首次出现顺序并深拷贝返回，故前端可以稳定显示顶层 `citations`，不依赖全部 Session
历史 Trace。

本次 Demo 文档只模拟“已完成入库并可查询”的知识库。未来生产环境应把入库链路放入
`DocumentIngestionService`，持久化 Document、Chunk 和索引状态；聊天查询前由认证授权层
...
### 10.3 已完成：Next.js Agent 控制台 MVP

项目新增 `frontend/`，使用 Next.js App Router、React、严格 TypeScript、Tailwind、TanStack
Query、Zustand 与 Vitest。浏览器只请求同源 `/api/...`；Next.js Route Handler 在服务端读取
`AGENT_API_BASE_URL` 并代理 Python API，保留后端 HTTP 状态码，并安全处理超时、不可连接和非 JSON
响应。前端不暴露 Python 后端地址，也不迁移 Agent 执行逻辑。

控制台以 ChatTurn 保存用户输入、回答、Citation、Node Trace、Tool Trace、状态和错误，并按
session_id 隔离。右侧 Trace 面板与历史回合选择绑定；Retrieval Trace 从任意
`retrieval.search` Tool Trace 的 `result.retrieval_trace` 读取，缺失该字段时仍能查看其他 Trace。
